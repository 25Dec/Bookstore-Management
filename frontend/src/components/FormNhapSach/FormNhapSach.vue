		...
		setup() {
			...
			let errOfSoLuongNhap = computed(() => {
				if (caiDatStore.suDungQuiDinh == 1) {
					if (soLuong.value < caiDatStore.soLuongNhapToiThieu && soLuong.value != "")
			...
			let errOfDonGia = computed(() => {
				if (donGia.value < 1000 && donGia.value != "") return `Giá tiền phải lớn hơn 1000`;
			});
			...
			let booksFilteredBySoLuongTonToiThieu = computed(() => {
				return sachStore.listOfBooks.filter((sach) => {
					if (sach.SoLuong < caiDatStore.luongTonToiThieuTruocKhiNhap) return sach;
				});
			});
	...
	>
		...
		>
			...
			<div>
				...
				/>
				<p style="font-size: 14px; color: red; font-style: italic">{{ errOfSoLuongNhap }}</p>
			</div>
			<div>
				...
				/>
				<p style="font-size: 14px; color: red; font-style: italic">{{ errOfDonGia }}</p>
			</div>
			<button>Lưu</button>
		</form>
	</div>
